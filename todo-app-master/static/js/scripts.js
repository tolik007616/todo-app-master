  ...
  $.ajax({
   ...
   success: function (data, status, xhr) {
    if (data['status']=='success'){
      ...
      $('.task'+task_id).after(task_append(data['swapid'], data['swapname'], data['date']));
      if (data['checked']){
 ...
 $('.projects').on("click", ".prdown", function(event) {
  ...
  $.ajax({
   ...
   success: function (data, status, xhr) {
    if (data['status']=='success'){
      $(".task"+task_id).remove();
      $('.task'+data['swapid']).after(task_append(task_id, data['name'], data['date']));
      if (data['checked']){
...
function task_date(){
  $(".projects").on('change',"[type='date'][task_id]", function(event){
  var btn = $(this);
  var date = $(this).val();
  var task_id = btn.attr('task_id');  
  var myurl = "/task_date/";  
...
function project_date(){
  $(".projects").on('change',"[type='date'][project_id]", function(event){
  var btn = $(this);
  var date = $(this).val();
  var project_id = btn.attr('project_id');  
  var myurl = "/project_date/";  
  $.ajax({
   type: 'POST',
   async: true,
   dataType: 'json',
